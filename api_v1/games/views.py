...
async def create_game_view(game: GameCreate, user: User = Depends(get_user_by_token), session: AsyncSession = Depends(db_helper.session_getter)) -> Game | HTTPException:
    if not user.role_access == 'Super user':
        return HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='You are not authorized to view this page.')
    return await create_game(game=game, session=session)

@router.delete('/delete_game/{game_id}')
async def delete_game_view(game_id: int, user: User = Depends(get_user_by_token), game: Game = Depends(get_game_by_id_dependency), session: AsyncSession = Depends(db_helper.session_getter)):
    if not user.role_access == 'Super user':
        return HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='You are not authorized to view this page.')
...
async def update_game_partial_view(game_id: int, game_data: GameUpdatePartial, user: User = Depends(get_user_by_token), game: Game = Depends(get_game_by_id_dependency), session: AsyncSession = Depends(db_helper.session_getter)) -> Game | HTTPException:
    if not user.role_access == 'Super user':
        return HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='You are not authorized to view this page.')
...
async def update_game_full_view(game_id: int, game_data: GameUpdateFull, user: User = Depends(get_user_by_token), game: Game = Depends(get_game_by_id_dependency), session: AsyncSession = Depends(db_helper.session_getter)) -> Game | HTTPException:
    if not user.role_access == 'Super user':
        return HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='You are not authorized to view this page.')
