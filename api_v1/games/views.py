...
async def create_game_view(
    game: GameCreate,
    user: User = Depends(get_user_by_token),
    session: AsyncSession = Depends(db_helper.session_getter),
) -> Game | HTTPException:
    if super_user_validate(user=user):
        return await create_game(game_info=game, session=session)


@router.delete("/delete_game/{game_id}")
async def delete_game_view(
    game_id: int,
    user: User = Depends(get_user_by_token),
...
async def update_game_partial_view(
    game_id: int,
    ...
    if super_user_validate(user=user):
        return await update_game_partial(
            game_to_update=game, game_data=game_data, session=session
        )
...
async def update_game_full_view(
    game_id: int,
...
@cache(expire=60)
async def get_game_info_by_id_view(
    game_id: int,
    session: AsyncSession = Depends(db_helper.session_getter),
    user: User = Depends(get_user_by_token),
    game: Game = Depends(get_game_by_id_dependency),
...
@cache(expire=60)
async def get_genres_info_by_game_id_view(
    game_id: int,
    session: AsyncSession = Depends(db_helper.session_getter),
    user: User = Depends(get_user_by_token),
    game: Game = Depends(get_game_by_id_dependency),
