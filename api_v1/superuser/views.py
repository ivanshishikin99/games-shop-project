...
async def login_super_user_view(form_data: OAuth2PasswordRequestForm = Depends(), session: AsyncSession = Depends(db_helper.session_getter)) -> TokenModel | HTTPException:
    user = await super_user_login(username=form_data.username, password=form_data.password, session=session)
