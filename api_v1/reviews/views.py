...
async def create_review_view(game_id: int,
                             rating: int,
                             description: str,
                             session: AsyncSession = Depends(db_helper.session_getter),
                             user: User = Depends(get_user_by_token)) -> Review:
    if not user.verified:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail="Your email has to be verified in order for you to leave reviews.")
    review = ReviewCreate(rating=rating,
                          description=description,
                          game_id=game_id,
                          user_id=user.id)
    return await create_review(review=review,
                               session=session)
...
async def delete_review_view(review_id: int,
                             review: Review = Depends(get_review_by_id_dependency),
                             user: User = Depends(get_user_by_token),
                             session: AsyncSession = Depends(db_helper.session_getter)):
    if not user.id == review.user_id and user.role_access != "Super user":
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="You cannot delete other users' reviews.")
    return await delete_review(review=review,
                               session=session)